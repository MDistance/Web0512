...
router.get('/getOpenId',async (ctx,next)=>{
	...
	console.log('浏览器端携带的请求参数的数据：code',code);
	...
	console.log('微信服务器返回的数据: ',  result.data);
		  ...
		  // 4自定义登录状态,对openId进行加密,生成token,jsonwebtoken jwt
		  let openId=result.openid;
		  // 5 jsonwebtoken对openid进行加密
		  // let token=jwt.sign(data,secret);生成token
		  // let data=jwt.verify(token,secret);注意反编译token的密钥必须和当初生成token的密钥一样
		  let token=jwt.sign(openId,'jiajingwen')
		  
		  
		  反编译token练习
		  let test =jwt.verify(token);没有密钥:secret  or  public key must be provided,
		  let test=jwt.verify(token,'luhan');密钥错误:JsonWebTokenError:invalid signature,
